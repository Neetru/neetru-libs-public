...
export const CONCEPT_GUIDE = Object.freeze({
    ...
    tenant: {
        ...
        summary: 'Unidade de acesso/billing/configuração de produto para um customer em um environmentId.',
        storageCollection: 'tenants',
        /** @deprecated Use storageCollection. Removido em próxima major. */
        collection: 'tenants',
        envIds: ['dev-local', 'staging', 'prod'],
        /** @deprecated 'dev' é alias legado — use 'dev-local'. */
        legacyEnvAlias: { dev: 'dev-local' },
    ...
    workspace: {
        ...
        summary: 'Faceta de runtime/motor isolado do tenant em `tenants/{tenantId}`. ' +
 ...
 */
export const DB_STATE = Object.freeze({
 ...
 */
export const ALLOWED_FOREIGN = new Set([
    'api',
    'json',
    'sql',
    'url',
 ...
 */
export function engineSeal(engine) {
    if (typeof engine !== 'string')
        return '';
    if (engine.startsWith('vm-'))
        return 'Econômico';
    if (engine.startsWith('cloud-sql-'))
        return 'Gerenciado';
 ...
 */
export const ENV_IDS = Object.freeze([
    'dev-local',
    'staging',
    'prod',
]);
 ...
 */
export function normalizeEnvId(input) {
    if (typeof input !== 'string') {
        throw new Error(`normalizeEnvId: ambiente inválido (esperava string, recebeu ${typeof input}). ` +
    ...
    if (!canonical) {
        throw new Error(`normalizeEnvId: ambiente "${input}" não reconhecido. ` +
