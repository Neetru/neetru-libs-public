...
import { STATEMENT_RULES, type RuleKey } from './rules.js';
import type {
 ...
 */
const ADDITIVE_CREATE_KEYWORDS: Readonly<Record<string, RuleKey>> = {
  type: 'CREATE_TYPE',
  schema: 'CREATE_SCHEMA',
  sequence: 'CREATE_SEQUENCE',
  domain: 'CREATE_DOMAIN',
};
const ADDITIVE_CREATE_LABELS: Readonly<Record<string, string>> = {
  type: 'tipo',
  schema: 'schema',
  sequence: 'sequence',
  domain: 'dominio',
...
/**
 * Resultado intermediario da classificacao de uma unica acao DDL.
 */
interface RuleHit {
  rule: RuleKey;
  reason: string;
 ...
 */
const SUGGESTIONS: Partial<Record<RuleKey, string>> = {
  DROP_COLUMN:
    'Padrao expand-contract: pare de usar a coluna no codigo e faca o deploy primeiro; remova a coluna num apply posterior.',
  DROP_TABLE:
    'Padrao expand-contract: garanta que nada mais le ou escreve na tabela e faca o deploy primeiro; remova a tabela num apply posterior. Considere fazer backup antes.',
  ALTER_COLUMN_TYPE:
    'Padrao expand-contract: crie uma coluna nova com o tipo desejado, copie os dados, migre o codigo e so depois remova a coluna antiga. Mudar o tipo in-place pode reescrever a tabela inteira e travar a aplicacao.',
  ADD_COLUMN_NOT_NULL_WITHOUT_DEFAULT:
    'Adicione uma clausula DEFAULT (a coluna passa a ser aditiva), ou faca em dois passos: 1) adicione a coluna como nullable e preencha os dados; 2) num apply posterior aplique o NOT NULL.',
  RENAME_COLUMN:
    'Padrao expand-contract: adicione a coluna nova, escreva nas duas, migre as leituras e o codigo, e so entao remova a coluna antiga. Renomear quebra qualquer codigo que ainda usa o nome antigo.',
  DROP_CONSTRAINT:
    'Confirme que nenhuma garantia de integridade depende dessa constraint antes de remove-la. Se for recriar com outra definicao, faca isso de forma controlada num apply dedicado.',
  UNKNOWN:
    'O classificador nao reconheceu este statement com seguranca. Revise manualmente: por seguranca (fail-closed) ele e tratado como destrutivo e exige confirmacao humana.',
};
 ...
 */
function tableName(node: unknown): string | null {
...
function classifyAlterAction(
  ...
  if (act === 'add' && resource === 'column') {
    ...
    const isNotNull =
      ...
      String((nullable as Record<string, unknown>)['type'] ?? '')
        .toLowerCase()
        .includes('not null');
    const hasDefault =
      action['default_val'] != null &&
  ...
  if (act === 'add' && resource === 'constraint') {
    ...
    const cLabel =
      typeof constraint === 'string' ? `"${constraint}"` : 'desconhecida';
    return {
      rule: 'ADD_CONSTRAINT',
 ...
 */
function classifyAst(ast: Record<string, unknown>): StatementClassification {
  ...
  if (type === 'alter') {
    ...
    const actions: Record<string, unknown>[] = Array.isArray(exprRaw)
      ? (exprRaw as Record<string, unknown>[])
    ...
    const destructive = hits.find(
      (h) => STATEMENT_RULES[h.rule] === 'destrutiva',
    );
    if (destructive) {
      if (hits.length === 1) return finalize(destructive.rule, destructive);
      const reasons = hits.map((h) => h.reason).join(' ');
      return finalize('UNKNOWN' as RuleKey, {
        rule: destructive.rule,
        reason: `ALTER TABLE com multiplas acoes, ao menos uma destrutiva. ${reasons}`,
      });
    }
    // todas aditivas
    if (hits.length === 1) return finalize(hits[0]!.rule, hits[0]!);
    const reasons = hits.map((h) => h.reason).join(' ');
 ...
 */
function finalize(rule: RuleKey, hit: RuleHit): StatementClassification {
  const severity: StatementSeverity = STATEMENT_RULES[rule];
 ...
 */
function detectUnparseableRename(rawSql: string): RuleHit | null {
  const normalized = rawSql.replace(/\s+/g, ' ').trim().toUpperCase();
  if (/^ALTER\s+TABLE\s+.+\bRENAME\s+COLUMN\b/.test(normalized)) {
    return {
      rule: 'RENAME_COLUMN',
      reason:
        'Renomeia uma coluna (ALTER TABLE ... RENAME COLUMN) — quebra todo codigo que ainda usa o nome antigo.',
    };
  }
  if (/^ALTER\s+TABLE\s+.+\bRENAME\s+TO\b/.test(normalized)) {
    return {
      rule: 'RENAME_COLUMN',
      reason:
        'Renomeia uma tabela (ALTER TABLE ... RENAME TO) — quebra todo codigo que ainda usa o nome antigo.',
    };
  }
  return null;
}
 ...
 */
function detectUnparseableAdditiveCreate(rawSql: string): RuleHit | null {
  ...
  if (/\b(DELETE|UPDATE|INSERT|MERGE|TRUNCATE)\b/i.test(normalized)) return null;
  const kw = m[1]!.toLowerCase();
  const ruleByKw: Record<string, RuleKey> = {
    table: 'CREATE_TABLE',
    type: 'CREATE_TYPE',
    schema: 'CREATE_SCHEMA',
    sequence: 'CREATE_SEQUENCE',
  ...
  const labelByKw: Record<string, string> = {
    table: 'tabela',
    type: 'tipo',
    schema: 'schema',
    sequence: 'sequence',
 ...
 */
function aggregateNodeClassifications(
  results: StatementClassification[],
): Omit<StatementClassification, 'sql'> {
 ...
 * multi-AST.
 */
export function classifyOneStatement(rawSql: string): StatementClassification {
  ...
  if (
    /^CREATE\s+(?:TABLE|MATERIALIZED\s+VIEW)\b/i.test(normForCtas) &&
    ...
    return {
      ...
      severity: 'destrutiva',
      reason:
        'CREATE ... AS (CTAS) com CTE/subquery que modifica dado (DELETE/UPDATE/INSERT/MERGE/TRUNCATE) — executa DML e altera dado existente; tratado como destrutivo por seguranca (fail-closed).',
  ...
  try {
    ...
    const nodes: unknown[] = Array.isArray(ast) ? ast : [ast];
    ...
    if (validNodes.length === 0) {
      return {
        ...
        severity: 'destrutiva',
        reason:
          'Nao foi possivel obter um AST utilizavel para este statement — tratado como destrutivo por seguranca (fail-closed).',
        suggestion: SUGGESTIONS.UNKNOWN!,
      };
    }
    ...
    if (validNodes.length !== nodes.length) {
      return {
        ...
        severity: 'destrutiva',
        reason:
          'O parser devolveu nos de AST nao utilizaveis neste chunk — tratado como destrutivo por seguranca (fail-closed).',
        suggestion: SUGGESTIONS.UNKNOWN!,
      };
    }
    ...
    return {
      ...
      severity: 'destrutiva',
      reason:
        'Statement SQL nao pode ser parseado (sintaxe nao reconhecida pelo parser) — tratado como destrutivo por seguranca (fail-closed).',
      suggestion: SUGGESTIONS.UNKNOWN!,
    };
  }
}
 ...
 */
export function classifyMigration(sql: string): ClassificationReport {
  ...
  const rawStatements = splitSqlStatements(input).filter(
    (s) => !isEffectivelyEmpty(s),
  );
  ...
  const overallSeverity: StatementSeverity = statements.some(
    (s) => s.severity === 'destrutiva',
  )
    ? 'destrutiva'
    : 'aditiva';
 ...
 */
export function explainStatement(c: StatementClassification): string {
