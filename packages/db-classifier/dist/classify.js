 ...
 */
function stripEdgeComments(sql) {
    const EDGE = '(?:\\s|--[^\\n]*(?:\\n|$)|/\\*[\\s\\S]*?\\*/)*';
 ...
 */
function detectIdempotentCreateTypeGuard(rawSql) {
    const normalized = rawSql.replace(/\s+/g, ' ').trim();
    const re = /^DO\s*\$\$\s*BEGIN\s*(CREATE\s+TYPE\s+(?:"[^"]+"\.)?"[^"]+"\s+AS\s+ENUM\([^;]*\);)\s*EXCEPTION\s+WHEN\s+duplicate_object\s+THEN\s+null;\s*END\s*\$\$;?$/i;
    const m = re.exec(normalized);
    if (!m)
        return null;
    return {
        rule: 'CREATE_TYPE',
        reason: 'Cria um tipo (CREATE TYPE) dentro de um guard idempotente ' +
 ...
 */
function detectIdempotentAddConstraintGuard(rawSql) {
    const normalized = rawSql.replace(/\s+/g, ' ').trim();
    const re = /^DO\s*\$\$\s*BEGIN\s*(ALTER\s+TABLE\s+(?:"[^"]+"\.)?"[^"]+"\s+ADD\s+CONSTRAINT\s+"[^"]+"\s+(?:FOREIGN\s+KEY|CHECK|UNIQUE|PRIMARY\s+KEY)[^;]*;)\s*EXCEPTION\s+WHEN\s+duplicate_object\s+THEN\s+null;\s*END\s*\$\$;?$/i;
    const m = re.exec(normalized);
    if (!m)
        return null;
    return {
        rule: 'ADD_CONSTRAINT',
        reason: 'Adiciona uma constraint (FK/CHECK/UNIQUE/PK) dentro de um guard ' +
