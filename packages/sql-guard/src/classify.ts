...
function unsafe(
  ...
  const verdict: SqlVerdict = { safe: false, statementType, reason };
 ...
 */
function safe(statementType: string, reason: string): SqlVerdict {
 ...
 */
function classifyExplain(sql: string, dialect: Dialect): SqlVerdict {
  ...
  if (inner.trim().length === 0) {
    return unsafe(
      'explain',
      'EXPLAIN sem statement interno reconhecivel — recusado por seguranca (fail-closed).',
    );
  }
  ...
  if (!innerVerdict.safe) {
    return unsafe(
      'explain',
      `EXPLAIN de um statement que nao e read-only (${innerVerdict.statementType}): ${innerVerdict.reason}`,
      innerVerdict.dangerousFunctions,
    );
  }
  return safe(
    'explain',
    'EXPLAIN de um statement read-only — apenas mostra o plano de execucao, seguro.',
  );
}
...
export function classifyStatement(
  ...
  if (typeof sql !== 'string' || sql.trim().length === 0) {
    return unsafe(
      'unknown',
      'Input SQL vazio — recusado por seguranca (fail-closed).',
    );
  }
  ...
  if (/\/\*!/.test(trimmed)) {
    return unsafe(
      'unknown',
      'SQL contem comentario executavel MySQL (/*! ... */) cujo conteudo o servidor executaria escondido das guardas — recusado por seguranca (fail-closed).',
    );
  }
  ...
  if (kw === 'copy') {
    ...
    return unsafe(
      'copy',
      'COPY le ou escreve arquivos do servidor — sempre recusado, nao e uma consulta de leitura.',
    );
  }
  ...
  if (/\binto\s+(?:out|dump)file\b/i.test(stripSqlComments(trimmed))) {
    return unsafe(
      'select',
      'SELECT ... INTO OUTFILE/DUMPFILE escreve arquivos do servidor — sempre recusado, nao e uma consulta de leitura.',
    );
  }
  ...
  try {
    ...
    return unsafe(
      'unknown',
      'SQL nao pode ser parseado (sintaxe nao reconhecida) — recusado por seguranca (fail-closed).',
    );
  }
  ...
  if (Array.isArray(ast)) {
    if (ast.length === 0) {
      return unsafe(
        'unknown',
        'O parser nao produziu nenhum statement — recusado por seguranca (fail-closed).',
      );
    }
    if (ast.length > 1) {
      return unsafe(
        'multiple',
        `Input contem ${ast.length} statements — apenas um unico statement read-only e permitido.`,
      );
    }
    ast = ast[0];
  }
  ...
  if (ast == null || typeof ast !== 'object') {
    return unsafe(
      'unknown',
      'O parser devolveu um AST nao utilizavel — recusado por seguranca (fail-closed).',
    );
  }

  const node = ast as Record<string, unknown>;
  const type =
    typeof node['type'] === 'string' ? node['type'].toLowerCase() : 'unknown';
  ...
  if (dangerous.length > 0) {
    return unsafe(
      ...
      `O statement referencia ${dangerous.length === 1 ? 'a funcao perigosa' : 'as funcoes perigosas'} ${dangerous
        .map((f) => `\`${f}\``)
        .join(', ')} — recusado.`,
      dangerous,
    );
  ...
  if (writeNode) {
    return unsafe(
      type,
      `O statement contem uma operacao de escrita (${writeNode.toUpperCase()}) — possivelmente uma CTE data-modifying. Recusado.`,
    );
  }
  ...
  return unsafe(
    type,
    `Statement do tipo \`${type}\` nao e read-only — apenas SELECT, EXPLAIN e WITH...SELECT sao permitidos.`,
  );
}
