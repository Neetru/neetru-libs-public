...
import { classifyStatement, type Dialect } from './classify.js';
 ...
 */
export function hardenSelect(sql: string, opts: HardenOptions): string {
  const { maxRows } = opts;
  const dialect: Dialect = opts.dialect ?? 'postgresql';
  ...
  if (
    ...
    throw new Error(
      `hardenSelect: maxRows precisa ser um inteiro positivo (recebido: ${String(maxRows)}).`,
    );
  }
  ...
  if (!verdict.safe) {
    throw new Error(
      `hardenSelect: o SQL nao e um SELECT seguro (${verdict.statementType}): ${verdict.reason}`,
    );
  }
  ...
  if (verdict.statementType !== 'select' && verdict.statementType !== 'with') {
    throw new Error(
      `hardenSelect: so aceita SELECT ou WITH...SELECT (recebido: ${verdict.statementType}).`,
    );
  }
