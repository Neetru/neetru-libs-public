 ...
 */
function sanitizeMaxConnections(maxConnections) {
    if (maxConnections === undefined) {
        return DEFAULT_MAX_CONNECTIONS;
    }
    if (typeof maxConnections !== 'number' || !Number.isFinite(maxConnections) || maxConnections < 1) {
        throw new Error(`computePoolSizing: maxConnections invalido — precisa ser um numero finito >= 1 ` +
 ...
 */
function validateNBancos(nBancos) {
    if (typeof nBancos !== 'number' ||
        !Number.isInteger(nBancos) ||
        nBancos < 1 ||
        nBancos > MAX_N_BANCOS) {
        throw new Error(`computePoolSizing: nBancos invalido — precisa ser um inteiro 1..${MAX_N_BANCOS} ` +
 ...
 */
export function computePoolSizing(nBancos, maxConnections) {
...
export function assertConnectionInvariant(input) {
    ...
    if (!isPositiveInteger(sdkPoolMax)) {
        throw new Error(`assertConnectionInvariant: sdkPoolMax deve ser um inteiro positivo, recebeu ${String(sdkPoolMax)}.`);
    }
    if (!isPositiveInteger(maxInstances)) {
        throw new Error(`assertConnectionInvariant: maxInstances deve ser um inteiro positivo, recebeu ${String(maxInstances)}.`);
    }
    if (!isPositiveInteger(defaultPoolSize)) {
        throw new Error(`assertConnectionInvariant: defaultPoolSize deve ser um inteiro positivo, recebeu ${String(defaultPoolSize)}.`);
    }
    const demanded = sdkPoolMax * maxInstances;
    if (demanded > defaultPoolSize) {
        throw new Error(`Invariante de conexao violada: sdkPoolMax (${sdkPoolMax}) x maxInstances ` +
