...
import type { MaskOptions, MaskPolicy, MaskedResultSet } from './types.js';
...
const VALID_POLICIES: ReadonlySet<MaskPolicy> = new Set<MaskPolicy>([
  'email',
  'cpf',
  'phone',
  'card',
 ...
 */
function resolvePolicy(colName: string, columns: Record<string, MaskPolicy>): MaskPolicy {
...
function maskEmail(value: unknown): string {
  if (typeof value !== 'string') return GENERIC_MASK;

  const at = value.indexOf('@');
  // Precisa de exatamente um '@'.
  if (at === -1 || value.indexOf('@', at + 1) !== -1) return GENERIC_MASK;

  const local = value.slice(0, at);
  const domain = value.slice(at + 1);
  if (local.length === 0 || domain.length === 0) return GENERIC_MASK;

  // O dominio precisa de ao menos um ponto (nome + TLD).
  const lastDot = domain.lastIndexOf('.');
  if (lastDot <= 0 || lastDot === domain.length - 1) return GENERIC_MASK;
...
function maskCpf(value: unknown): string {
  ...
  if (digits.length !== 11) return GENERIC_MASK;
...
function maskPhone(value: unknown): string {
  ...
  if (totalDigits < 4) return GENERIC_MASK;
...
function maskCard(value: unknown): string {
  if (typeof value !== 'string') return GENERIC_MASK;
  const digits = digitsOf(value);
  if (digits.length < 12 || digits.length > 19) return GENERIC_MASK;
 ...
 */
function applyPolicy(value: unknown, policy: MaskPolicy): unknown {
...
function maskValueRecursive(
  ...
  if (Array.isArray(value)) {
    ...
    return value.map((item) =>
      maskValueRecursive(item, wildcardPath, columns, masked, depth + 1),
    );
  }
...
export function maskResultSet(
  ...
  const maskedColumnsSet = new Set<string>();
  const maskedRows: Record<string, unknown>[] = new Array(rows.length);
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    for (const colName of Object.keys(source)) {
      ...
      if (
        ...
        const innerMasked = new Set<string>();
        next[colName] = maskValueRecursive(
          colValue,
          colName,
          columns,
          innerMasked,
